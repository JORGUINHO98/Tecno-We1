...
<div class="max-w-2xl mx-auto">
    ...
    <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-10 shadow-2xl">
        <h1 class="text-4xl font-orbitron tracking-wider bg-gradient-to-r from-neon-cyan to-neon-purple bg-clip-text text-transparent drop-shadow-neon mb-12 text-center">
        ...
        <form method="POST" action="{{ $producto ? route('producto.update', $producto->id) : route('producto.store') }}" class="space-y-8">
            @csrf
            @if($producto)
                @method('PUT')
            @endif
            ...
            <div>
                ...
                <textarea name="descripcion" rows="5" class="w-full p-5 backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl font-medium text-lg focus:ring-4 ring-neon-cyan focus:border-neon-cyan transition-all @error('descripcion') border-red-400 @enderror" 
                          placeholder="Describe las características del producto..." required>{{ old('descripcion', $producto->descripcion ?? '') }}</textarea>
            </div>
