...
<div class="max-w-2xl mx-auto">
    <div class="text-center mb-12">
        <h1 class="text-5xl font-orbitron tracking-wider bg-gradient-to-r from-neon-cyan to-neon-purple bg-clip-text text-transparent drop-shadow-neon mb-4">EDITAR CLIENTE</h1>
        <a href="{{ route('cliente.index') }}" class="btn-outline-neon px-6 py-2 rounded-xl font-medium">← Volver a Lista</a>
    </div>

    <form method="POST" action="{{ route('cliente.update', $cliente) }}" class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-10 shadow-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="direccion" rows="4" required class="w-full backdrop-blur-xl bg-white/20 border border-white/30 rounded-2xl px-6 py-4 text-white placeholder-gray-400 focus:border-neon-cyan focus:ring-2 focus:ring-neon-cyan/50 transition-all font-medium">{{ old('direccion', $cliente->direccion) }}</textarea>
            </div>
        </div>

        <div class="flex space-x-4 mt-10">
            <button type="submit" class="flex-1 btn-neon py-4 rounded-2xl text-lg font-semibold hover:scale-105">Actualizar Cliente</button>
